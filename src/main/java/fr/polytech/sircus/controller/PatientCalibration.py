...
def calibrateAndValidate():
    ...
    for point in points_to_calibrate:
        ...
        width_diff = round(canvas.coords(big_circle).__getitem__(0), 1) - (root.winfo_screenwidth() * point[0] - 20)
        if width_diff > 0:
            width_direction = -1
        elif width_diff == 0:
            width_direction = 0
        ...
        height_diff = round(canvas.coords(big_circle).__getitem__(1), 1) - (root.winfo_screenheight() * point[1] - 20)
        if height_diff > 0:
            height_direction = -1
        elif height_diff == 0:
            height_direction = 0
        ...
        while round(canvas.coords(big_circle).__getitem__(0), 1) != (root.winfo_screenwidth() * point[0] - 20):
            canvas.move(big_circle, width_direction * root.winfo_screenwidth() / 500,
                        height_direction * root.winfo_screenheight() / 500)
            canvas.move(little_circle, width_direction * root.winfo_screenwidth() / 500,
                        height_direction * root.winfo_screenheight() / 500)
            time.sleep(0.0001)
            canvas.update()
